...
// --- เปลี่ยนบรรทัดนี้ตามโหมดที่ใช้ ---
// public:   import TopBarPublic from "@/components/TopBarPublic";
// admin:    import TopBarAdmin  from "@/components/TopBarAdmin";
// officer:  import TopBarOfficer from "@/components/TopBarOfficer";
// teacher:  import TopBarTeacher from "@/components/TopBarTeacher";
// external: import TopBarExternal from "@/components/TopBarExternal";
...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  if (!p) {
    return (
      <main>
        {/* ถ้าเป็นโหมดอื่น ให้เปลี่ยน TopBar* ตรงนี้ */}
        <TopBarPublic />
        <Container sx={{ py: 4 }}>
  ...
  return (
    <main>
      ...
      <Container sx={{ py: 4 }}>
        ...
        {/* กล่องหลัก: มุมมน เงาอ่อน ขอบบาง */}
        <Paper
          elevation={0}
          sx={{
            p: { xs: 2, md: 3 },
        ...
        >
          <Grid container spacing={3}>
            {/* ซ้าย: การ์ดรายละเอียด */}
            <Grid size= {{xs: 12, md: 4}}>
              <Typography variant="subtitle1" fontWeight={800} sx={{ mb: 1.5 }}>
                รายละเอียดผลงาน
              </Typography>

              {/* กล่องรูป (สีเทา มุมโค้ง) */}
              <Box
                sx={{
                  width: "100%",
                  aspectRatio: "1 / 1",
              ...
              {/* ปุ่มดาวน์โหลด: กดได้ (เอฟเฟกต์) แต่ไม่มี action */}
              <Button
                type="button"
...
/** แถว label:value สไตล์เหมือนภาพ */
