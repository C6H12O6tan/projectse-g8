  ...
  return (
    <main>
      ...
      <Container className="container" sx={{ py: 3 }}>
        ...
        {/* ปุ่มค้นหาขั้นสูงอยู่ชิดขวาของช่องค้นหา */}
        <SearchBarRow rightLabel="ค้นหาขั้นสูง" rightVariant="outlined" rightHref="/teacher/search" />

        <Grid container spacing={2}>
          {ITEMS.map(it => (
            <Grid key={it.id} size={{ xs:12, sm:6, md:4 }}>
              <ProjectThumb href="/teacher/project" image={it.img} title={it.title} author={it.author} tag={it.tag}/>
            </Grid>
          ))}
        </Grid>
      </Container>
