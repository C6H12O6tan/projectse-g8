...
import TopBarTeacher from '@/components/TopBarTeacher';  // ← ไม่มีให้ใช้ TopBarPublic แทน
    ...
    <main>
      ...
      <Container className="container" sx={{ py: 3 }}>
        <Stack direction="row" alignItems="center" spacing={60} sx={{ mb: 1 }} flexWrap="wrap">
          ...
          <Box component="form" action="/teacher/search" sx={{ display: 'flex', gap: 1 }}>
            <TextField
              name="q"
        ...
        <Grid container spacing={2}>
          {items.map((it) => (
            <Grid key={it.id} size={{ xs: 12, sm: 6, md: 4 }}>
              <ProjectThumb
                href={`/teacher/publications/${it.id}`}
                title={it.title}
