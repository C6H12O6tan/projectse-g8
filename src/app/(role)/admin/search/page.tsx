...
const MOCK: Item[] = [
  ...
  { id: "2", title: "AI in Classroom Settings", author: "Floyd Miles", dept: "คณะวิศวกรรมศาสตร์", type: "บทความ", year: 2568 },
];
const DEPTS = ['ทั้งหมด',
  'คณะวิศวกรรมศาสตร์', 'คณะวิทยาศาสตร์', 'คณะแพทยศาสตร์', 'คณะพยาบาลศาสตร์',
  'คณะวิทยาการจัดการ', 'คณะทรัพยากรธรรมชาติ', 'คณะอุตสาหกรรมเกษตร', 'คณะทันตแพทยศาสตร์',
  'คณะเภสัชศาสตร์', 'คณะการแพทย์แผนไทย', 'คณะนิติศาสตร์', 'คณะเศรษฐศาสตร์',
  'คณะสัตวแพทยศาสตร์', 'วิทยาลัยนานาชาติ', 'บัณฑิตวิทยาลัย'
];
const TYPES = ["ทั้งหมด", "วิจัย", "บทความ", "รายงาน"];
const YEARS = ["ทั้งหมด", "2568", "2567", "2566", "2558"];

export default function AdminSearch() {
  const [q, setQ] = useState(""); const [dept, setDept] = useState(DEPTS[0]);
  const [type, setType] = useState(TYPES[0]); const [year, setYear] = useState<string | number>(YEARS[0]);

  const results = useMemo(() => MOCK.filter(it => {
      ...
      <Container className="container" sx={{ py: 3 }}>
        ...
        <Grid container spacing={1} sx={{ mb: 2 }}>
          <Grid size={{ xs: 12, md: 4 }}>
            <TextField fullWidth size="small" placeholder="ชื่อผลงาน,ชื่อผู้วิจัย…" value={q} onChange={e => setQ(e.target.value)}
        ...
        <Stack spacing={1.5}>
          {results.map(r => (
            <Paper key={r.id} elevation={0} sx={{ p: 2, border: `1px solid ${PSU.cardBorder}`, borderRadius: 2, boxShadow: PSU.cardShadow }}>
              ...
              <Stack direction="row" spacing={2} alignItems="center">
                ...
                <Stack sx={{ ml: "auto" }}>
                  <Button size="small" variant="contained" href={`/admin/history`}>ดูประวัติ</Button>
                </Stack>
              </Stack>
            </Paper>
          ))}
